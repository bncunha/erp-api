...
func (r *productRepository) Edit(ctx context.Context, product domain.Product, id int64) (int64, error) {
	...
	if product.Category.Id == 0 {
		query = `UPDATE products SET name = $1, description = $2, company_id = $3 WHERE id = $4 RETURNING id`
		args = []any{product.Name, product.Description, config.COMPANY_TEST_ID, id}
	} else {
		query = `UPDATE products SET name = $1, description = $2, company_id = $3, category_id = $4 WHERE id = $5 RETURNING id`
